...
\usepackage{amsmath,amssymb,amsthm,bm,booktabs,array,graphicx,float,enumitem,hyperref,fancyhdr,needspace}
\usepackage{tikz}
...
\newtheorem{proposition}{结论}
\input{paper_body_data.tex}
\begin{document}
\begin{center}
...
{\large\bfseries 摘要}
\end{center}
无线电干扰源的现场排查需要兼顾搜索范围、定位精度和清除时间。针对源数量、位置及发射方向未知的情况，本文利用带误差的示向度建立位置可行域，并将区域扫描和局部清除统一安排，形成机器狗的自动搜索策略。

针对问题一，将示向误差转化为正向楔形的半平面约束，通过可行性判别、边界交点枚举和顶点距离计算得到交会区域直径。构造边长36米的等边三角形算例，其最小包围圆半径为20.785米，说明以区域直径为直径的圆并不总能覆盖区域。进一步用最小包围圆建立一次清除的判断条件。

针对问题二，分析交会夹角、目标距离与定位误差的关系。根据首次接收信息，得到由三个半径1000米圆盘组成的交集作为第二检测点候选区域，再以第二次测向后的最坏包围圆半径评价42个候选点。算例中，横向候选的包围半径为\QTwoRadius{}米，共线对照为\QTwoControlRadius{}米。

针对问题三，以原点和正六边形环的六个点组成扫描骨架。环半径取1200米时，全域最坏覆盖距离为968.90米。结合联合任务排序、横向测向和矩形光学覆盖完成源的定位与清除，并利用逐频道扫描记录确定停止时机。在此基础上，引入路线组合、原地补测及全向负观测更新，构成增强策略field。

针对问题四，采用原点、940米内环和1870米错相外环组成的25点骨架。通过36个闭三角形的剖分，证明任意源位置和发射方向都能被至少一个检测点接收；信号中断后采用回退测向，并保留光学覆盖。每题1000个同世界合成案例中，field相对refined的平均单源时间分别降低\QThreeGain\%和\QFourGain\%。六次正式测试均使用field，问题三、四的三局等权平均时间分别为\FormalThreeMean{}和\FormalFourMean{}秒/源，均以频道完成判据结束。

\noindent\textbf{关键词：}交会定位；最小包围圆；全域覆盖；任务排序；定向干扰源
\clearpage
...
\end{equation}
左侧来自圆盘中任意两点距离不超过直径。若包围圆由两个点确定，则$r_*=D/2$；若由三个点确定，三个支持点中必有一对对应的圆心角不小于$120^\circ$，弦长不小于$\sqrt3r_*$，由此得到右侧。
...
新增一次观测时依次对当前凸多边形作半平面裁剪，保留满足条件的顶点与边界交点。若出现空域，说明观测或数值计算需要检查，当前频道不能据此记为无源。该表示同时提供局部测向的候选范围和清除位置判断。
...
\end{equation}
问题三取$\gamma=0$，按任务中心之间的行程排序。基准策略refined先用最近插入法形成完整排列，再进行有界2-opt改进：反转一段连续路线，若首尾连接费用下降则接受。每轮只执行得到的第一项任务，执行后重新计算位置区域和剩余任务。
...
按蛇形排列访问各中心，再将序列循环移位，使首点靠近当前位置。真实源在某个单元内，遍历过程中至少有一个中心落在它的清除范围内，收到成功反馈后立即结束该频道。
...
=54\times4=216
\end{equation}
个中心，其中$\varepsilon$取实现中的微小向内余量。实际可行域通常经过多次裁剪，所需网格少于此上界。有限测向次数和有限网格共同保证一个局部任务不会因持续无信号而无限循环；现实运行期限则由程序单独检查。
...
\bibitem{welzl} Welzl E. Smallest enclosing disks (balls and ellipsoids)[C]//New Results and New Trends in Computer Science. Lecture Notes in Computer Science, vol. 555. Springer, 1991: 359--370. DOI: 10.1007/BFb0038202.
\end{thebibliography}
\label{endbody}
\end{document}
